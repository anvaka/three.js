	...
	*/
	...
	for ( i = 1; i < numpoints; i ++ ) {
		...
		if ( vec.length() > Number.EPSILON ) {
			...
			theta = Math.acos( THREEMath.clamp( tangents[ i - 1 ].dot( tangents[ i ] ), - 1, 1 ) ); // clamp for floating pt errors
	...
	if ( closed ) {
		...
		theta = Math.acos( THREEMath.clamp( normals[ 0 ].dot( normals[ numpoints - 1 ] ), - 1, 1 ) );
		theta /= ( numpoints - 1 );
